	...
	addFormClick: function(){
		...
		if(this.getForm().isValid()){
			...
			this.items.each(function(item,index,length){ 
				console.log(item.getXType());
					if((""+item.getXType()).indexOf("field") != -1 && item.getValue() != '') {
						if (item.getXType() == 'numberfield') {
							if(null==item.getValue()||""==item.getValue()){
//								sum+=0;
							}else{
								sum+=parseFloat(item.getValue());
							}
			...
			if(sum!=(parseFloat(this.amount.getValue())*2).toFixed(2)){
				Ext.Msg.alert("系统提示：","清分总额度与清分余额不一致  ！");
				return false;
			}
			this.getForm().submit({
				waitMsg: '正在提交数据...',
...
//		this.amount = this.createTextField('账户余额','amount','95%','',null,21,'长度超过不能21');
		...
		this.itemId1 = (new Ext.form.ComboBox({
			fieldLabel:'科目一',
				name:"itemId1",
				mode:'local',
				allowBlank:true,
		...
		this.itemId2 = (new Ext.form.ComboBox({
			fieldLabel:'科目二',
			name:"itemId2",
			mode:'local',
			allowBlank:true,
		...
		this.item1Amt = new Ext.form.NumberField({
            fieldLabel: '<font color="red">*</font>科目一金额',
            name: 'item1Amt',
            id:'item1Amt',
            allowBlank: true,
//          allowBlank: false,
		...
		this.item2Amt = new Ext.form.NumberField({
            fieldLabel: '<font color="red">*</font>科目二金额',
            name: 'item2Amt',
            id:'item2Amt',
            allowBlank: true,
//          allowBlank: false,
...
//                   this.id_
	...
	addFormClick: function(){
		...
		var itemId1 = this.itemId1.getValue();
		var itemId2 = this.itemId2.getValue();
//		var itemId3 = this.itemId3.getValue();
		var item1Amt = this.item1Amt.getValue();
		var item2Amt = this.item2Amt.getValue();
//		var item3Amt = this.item3Amt.getValue();
		if(itemId1==itemId2){
			Ext.Msg.alert("系统提示：","不能有2个相同的科目 ！");
			return false;
		}
...
//		alert(sum+"=====?"+rzAmt);
		if(sum!=rzAmt*2){
			Ext.Msg.alert("系统提示：","入账总额度与分配额总和不一致  ！");
			return false;
		}
			...
			Ext.Msg.confirm('系统提示：',"入账 金额 为 ："+this.rzAmt.getValue(),function(btn){
				if(btn == 'yes'){
					...
					Ext.Ajax.request({
							...
							url: DQF_URL_RZ,
							method: 'POST',
//							params: {accNo:accNo,rzAmt:rzAmt,date:date,itemId1:itemId1,
//								itemId2:itemId2,itemId3:itemId3,item1Amt:item1Amt,item2Amt:item2Amt,item3Amt:item3Amt},
							params: {accNo:accNo,rzAmt:rzAmt,date:date,itemId1:itemId1,
									itemId2:itemId2,item1Amt:item1Amt,item2Amt:item2Amt},
							success: function(form,action){
