...
var ACCOUNT_SYS_CODE = [{'id' : '0', 'name' : '全部'}, {'id' : '1', 'name' : '卫生院'}, {'id' : '2', 'name' : '医养中心'}];
...
/*
		...
		this.inOrOut = this.createMemoryCombo('<font color="red">*</font>收入支出标志1', 'id', 'name', '95%', INOROUT_DATA, 'inOrOut');
...
WjwAccountGrid = Ext.extend(UxGrid,{
	constructor: function(height,width){
		...
		this.vtbar = new Ext.Toolbar({
			items: [
			       ...
			       '-',{xtype:'button',text:'删除',iconCls:'delete',handler:this.onDeleteClick,scope:this},
					...
					// '-',{xtype:'label',text:'账户体系'},{xtype:'combo',id:'accCode',store:ACCOUNT_SYS_CODE,width : 90},
					'-',{xtype: 'combo',id:'accCode', mode: 'local', store: new Ext.data.ArrayStore({
							fields: [ 'id',   'name'],
							data: [['', '全部'], [1,'卫生院'], [2,'医养中心']]
						}),
						valueField: 'id',
						displayField: 'name',
						triggerAction: 'all',
			       ...
			       '-',{xtype:'button',text:'查询',iconCls:'query',handler:function(){
							...
							var accCode =  Ext.getCmp('accCode').getValue();
			       			wjwAccountGrid.store.baseParams= {
		    	   					//unitNo:unitNo,
			       					accNo:accNo,custName:custName,accCode:accCode
		    	   			};
			       			wjwAccountGrid.store.load({params:{start:0,limit:PAGESIZE}});
	    	   			}
	       			},
		...
		this.vcm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
		            ...
		            {header:'上一级账户号9',dataIndex:'accParent',width:180,sortable:true,hidden:false},
