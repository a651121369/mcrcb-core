var ENTITY_URL_LIST = "/AccountManager/pager";
var ENTITY_URL_SAVE = "/AccountManager/save";
var ENTITY_URL_UPDATE = "/AccountManager/update";
var ENTITY_URL_DELETE = "/AccountManager/delete";
var STATUS_DATA = [{'id' : '1', 'name' : '正常'}, {'id' : '2', 'name' : '注销'}];
...
var INTTYPE_DATA = [{'id' : '1', 'name' : '按月计息'}, {'id' : '2', 'name' : '按季计息'}, {'id' : '3', 'name' : '按年计息'}];
...
WjwAccount = Ext.extend(Ext.ux.Form,{
	constructor: function(){
		...
		this.accNo = this.createTextField('账号','accNo','95%','',null,32,'长度超过不能32');
		this.custName = this.createTextField('账户名称','custName','95%','',null,60,'长度超过不能60');
		...
		this.unitNo = this.createTextField('机构号','unitNo','95%','',null,20,'长度超过不能20');
		...
		this.accNum = this.createTextField('账户积数','accNum','95%','',null,21,'长度超过不能21');
		...
		this.accParent = this.createTextField('上一级账户号11','accParent','95%','',null,32,'长度超过不能32');
		...
		this.rate = this.createTextField('利率','rate','95%','',null,9,'长度超过不能9');
		this.bankUnit = this.createTextField('银行开户机构号','bankUnit','95%','',null,20,'长度超过不能20');
		this.bankName = this.createTextField('银行开户账户名称','bankName','95%','',null,60,'长度超过不能60');
		this.bankAmount = this.createTextField('银行余额','bankAmount','95%','',null,21,'长度超过不能21');
...
/*
		...
		this.inOrOut = this.createMemoryCombo('<font color="red">*</font>账户收入支出标志', 'id', 'name', '95%', INOROUT_DATA, 'inOrOut');
...
WjwAccountGrid = Ext.extend(UxGrid,{
	constructor: function(height,width){
		...
		this.vtbar = new Ext.Toolbar({
			items: [
			       ...
			       '-',{xtype:'button',text:'删除',iconCls:'delete',handler:this.onDeleteClick,scope:this},
					...
					'-',{xtype:'label',text:'账号'},{xtype:'textfield',id:'Q_accNo_S_LK'},
					'-',{xtype:'label',text:'账户名称'},{xtype:'textfield',id:'Q_custName_S_LK'},
		...
		this.vcm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
		            ...
		            {header:'账户名称1',dataIndex:'custName',width:100,sortable:true,hidden:false},
		            {header:'账户种类',dataIndex:'accType',width:100,sortable:true,hidden:false,
		            ...
		            {header:'账户余额',dataIndex:'amount',width:100,sortable:true,hidden:false},
		            {header:'账户积数',dataIndex:'accNum',width:100,sortable:true,hidden:false},
		            {header:'账户状态',dataIndex:'accStatus',width:100,sortable:true,hidden:false,
		            	renderer:function(data, metadata, record){
	                        if(data ==1){
		            			return '正常';
		            		}else if(data ==2){
		            ...
		            {header:'上一级账户号12',dataIndex:'accParent',width:100,sortable:true,hidden:false},
		            ...
		            {header:'账户级别',dataIndex:'level',width:100,sortable:true,hidden:false},
		            {header:'利率',dataIndex:'rate',width:100,sortable:true,hidden:false},
		            {header:'银行开户机构号',dataIndex:'bankUnit',width:100,sortable:true,hidden:false},
		            {header:'银行开户机构名称',dataIndex:'bankName',width:100,sortable:true,hidden:false},
		            {header:'银行余额',dataIndex:'bankAmount',width:100,sortable:true,hidden:false},
		            {header:'不明来账收入',dataIndex:'unkCome',width:100,sortable:true,hidden:false},
		            {header:'利息收入',dataIndex:'intCome',width:100,sortable:true,hidden:false},
		            {header:'币种',dataIndex:'currency',width:100,sortable:true,hidden:false},
		            {header:'账户属性',dataIndex:'accFld',width:100,sortable:true,hidden:false,
		            	renderer:function(data, metadata, record){
	                        if(data ==1){
		            			return '真实账户';
		            		}else if(data ==2){
		            ...
		            {header:'账户类型',dataIndex:'accPro',width:100,sortable:true,hidden:false,
		            	renderer:function(data, metadata, record){
	                        if(data ==1){
		            			return '活期';
		            		}else if(data ==2){
		            ...
		            {header:'计息标志',dataIndex:'intFlag',width:100,sortable:true,hidden:false,
	            		renderer:function(data, metadata, record){
		            ...
		            {header:'账户收入支出标志',dataIndex:'inOrOut',width:100,sortable:true,hidden:false,renderer:function(data, metadata, record){
                        if(data ==1){
	            			return '收入';
	            		}else if(data ==2){
	            			return '支出';
		            ...
		            {header:'医疗金额',dataIndex:'medicalAmt',width:100,sortable:true,hidden:false}
		            //{header:'其他金额',dataIndex:'otherAmt',width:100,sortable:true,hidden:false},
