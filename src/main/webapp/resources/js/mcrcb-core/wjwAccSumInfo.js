			...
			items: [
				...
				new Ext.form.ComboBox({
					...
					store: new Ext.data.Store({
						proxy: new Ext.data.HttpProxy({url: '/public/getAccCode', method: 'POST'}),
...
//			    	   					UNIT_NAME:UNIT_NAME
			        ...
			        '-',{xtype:'button',text:'导出Excel文件',iconCls:'excel',handler:function(){
			       			...
			       			Ext.Msg.confirm("系统提示：", "确定下载查询结果吗？", function(btn){
			       				...
			       				if(btn=="yes"){
									var UNIT_NO = Ext.getCmp('UNIT_NO').getValue();
									var accCode1 = Ext.getCmp('accCode').getValue();
			    	   			   var urla="/mcrcb-core/getWjwAccSumInfoDown?UNIT_NO="+UNIT_NO+"&accCode"+accCode1;
									var w  = window.open(urla,'_blank');
					    	       w.location.href = url;
