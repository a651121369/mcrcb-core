var ENTITY_URL_LIST = "/mcrcb-core/getCertNoInfo";
var PAGESIZE=50;
...
WjwPaymain = Ext.extend(Ext.ux.Form,{
	...
	updateFormClick: function(){
		if(this.getForm().isValid()){
			this.getForm().submit({
				waitMsg: '正在提交，请稍后...',
//				url: "/mcrcb-core/changeAccInfoByCertNo?connNo="+vrecord.data.connNo+"&UNIT_NAME="+vrecord.data.UNIT_NAME+
//   		   		"&UNIT_NO="+UNIT_NO+"&OUT_ACCNAME="+OUT_ACCNAME+"&OUT_ACCNAME="+OUT_ACCNAME+"&OUT_ACCNO="+OUT_ACCNO+
//   		   		"&IN_NAME="+IN_NAME+"&IN_ACCNO="+IN_ACCNO+"&AMOUNT="+AMOUNT+"&DRUG_AMT="+DRUG_AMT+"&MEDICAL_AMT="+MEDICAL_AMT,
				url: "/mcrcb-core/changeAccInfoByCertNo",
				method: 'POST',
				success: function(form,action){
					Ext.MessageBox.alert("系统提示：","记账成功！");
...
WjwPaymainGrid = Ext.extend(UxGrid,{
	constructor: function(height,width){
		...
		this.vtbar = new Ext.Toolbar({
			items: [
			        '-',{xtype:'button',text:'记账',iconCls:'edit',handler:this.onModifyClick,scope:this}, 
...
//					    value:2//默认选中转账
			        ...
			        '-',{xtype:'button',text:'查询',iconCls:'query',handler:function(){
		    	   		    ...
		    	   		    var tranType =  Ext.getCmp('tranType').getValue();
		    	   		    ...
		    	   		    wjwPaymainGrid.store.baseParams= {
		    	   		    		tranType:tranType,status:status,startTime:startTime,endTime:endTime,connNo:connNo,amount:amount
		    	   			};
		    	   		    wjwPaymainGrid.store.load({params:{start:0,limit:PAGESIZE}});
	    	   			}
	       			}
