var ENTITY_URL_LIST = "/AccountManager/pageropen";
var ENTITY_URL_SAVE = "/AccountManager/saveopen";
var ENTITY_URL_UPDATE = "/AccountManager/updateopen";
var ENTITY_URL_DELETE = "/AccountManager/deleteopen";
...
var ACCOUNT_SYS_CODE = [{'id' : '1', 'name' : '卫生院'}, {'id' : '2', 'name' : '医养中心'}];
var CURRENCY_DATA = [{'id' : '156', 'name' : '人民币'}];
//var ACCFLD_DATA = [{'id' : '1', 'name' : '真实账户'}, {'id' : '2', 'name' : '虚拟账户'}];
var ACCFLD_DATA = [{'id' : '1', 'name' : '真实账户'}];
...
WjwAccount = Ext.extend(Ext.ux.Form,{
	constructor: function(){
		...
		this.custName = this.createTextField('<font color="red">*</font>账户名称','custName','95%','',null,60,'长度超过不能60');
		//添加下拉框
		this.accCode = this.createMemoryCombo('<font color="red">*</font>账户体系','id', 'name', '95%', ACCOUNT_SYS_CODE, 'accCode');
...
WjwAccountGrid = Ext.extend(UxGrid,{
	constructor: function(height,width){
		...
		this.vtbar = new Ext.Toolbar({
			items: [
			       ...
			       '-',{xtype:'button',text:'删除',iconCls:'delete',handler:this.onDeleteClick,scope:this},
					...
					'-',{xtype:'label',text:'账户名称'},{xtype:'textfield',id:'custName'},
		...
		this.vcm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
		            ...
		            {header:'账号',dataIndex:'accNo',width:180,sortable:true,hidden:false},
		            {header:'账户名称',dataIndex:'custName',width:200,sortable:true,hidden:false},
					{header:'账户体系',dataIndex:'accCode',width:60,sortable:true,hidden:false,
		            ...
		            {header:'账户余额',dataIndex:'amount',width:140,sortable:true,hidden:false,align: 'right',
		            ...
		            {header:'上一级账户号',dataIndex:'accParent',width:180,sortable:true,hidden:false},
   		...
   		if(records.length > 0) {
   			if(records.length == 1){
   				...
   				if(vrecord.data.accFld == 1 ||vrecord.data.accFld=='1'){
					...
					winForm.accCode.setValue(vrecord.data.accCode);
